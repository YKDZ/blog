...
- 直接编辑 markdown 文件，不用为了写个博客就花时间适应那些 WYSIWYG 编辑器，且不用引入任何 MCP 就可以适配任何 agent
...
站点还仅在 SSG 期间自动生成 sitemap.xml、robots.txt、llms.txt、atom.xml 等辅助检索、阅读的静态文件，因此也不会给码字阶段引入任何额外的心智负担。
...
最后，`index.md` 和存储它的目录基本上完整描述了一篇博客的的所有数据，没有什么额外的配置文件可以定义一篇博客，这种文档优先的做法也进一步降低了码字难度。
